 ...
 * // other contributions:
 * //   - eric willis [list of
 * honorifics](http://notes.ericwillis.com/2009/11/common-name-prefixes-titles-and-honorifics/)
 * //   - `TomThak` for raising issue #16 and providing [wikepedia
 * resource](https://cs.wikipedia.org/wiki/Akademick%C3%BD_titul)
 * //   - `atla5` for closing the issue.
 */
class FullNameParserHelper
{
    /**
     ...
     * Note: longer professional titles should appear earlier in the array than shorter titles to reduce the risk of
     * mis-identification e.g. BEng before BE Also note that case and periods are part of the matching for professional
    ...
    public function parse_name($full_name)
    {
        ...
        if (count($unfiltered_name_parts)) {
            ...
            while (count($unfiltered_name_parts) > 0 && $s = $this->is_line_suffix(
                    $unfiltered_name_parts[count($unfiltered_name_parts) - 1],
                    $full_name
                )) {
                if ($suffix != "") {
                    $suffix = $s . ", " . $suffix;
    ...
    public function fix_case($word)
    {
        ...
        if (mb_strlen($word) == 2) {
            ...
            if (in_array(mb_strtolower($word[0]), $this->dict['vowels']) && in_array(
                    mb_strtolower($word[1]),
                    $this->dict['vowels']
                )) {
                $word = mb_strtoupper($word);
            }
